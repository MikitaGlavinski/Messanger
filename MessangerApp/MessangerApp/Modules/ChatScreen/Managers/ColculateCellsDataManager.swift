    ...
    private func handleImageMessage(with messageModel: MessageViewModel, configureWithDate: Bool) -> ImageCellData? {
        ...
        if !messageModel.isOwner {
            ...
            if configureWithDate {
                ...
                data.imageViewFrame = CGRect(x: UIScreen.main.bounds.width - (messageModel.previewWidth ?? 0) - paddingNumber + 3, y: 13, width: messageModel.previewWidth ?? 0, height: messageModel.previewHeight ?? 0)
            }
